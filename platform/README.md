...
启动完成后打开 `http://localhost:4173`，点击“进入本地开发环境”。“数据接入与分析”支持直接上传 UTF-8 CSV，最大 10 MB，必需列为 `原声id`、`原声内容`、`商品标题`；上传后后端会完成校验、去重、隐私脱敏、本地规则基线分析、主题聚合与机会草稿生成。规则基线会明确保持“待人工复核”，不冒充模型最终结论。
