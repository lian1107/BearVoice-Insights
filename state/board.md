...
- 驾驶舱已统一为单一产品决策视图；删除无内容差异的“赛事/企业”切换及其 API 参数，避免用户误以为存在两套数据口径。
- UI 已按“证据优先”方向完成整体改造：侧栏入口改为决策总览、产品机会、数据资产、主题治理、质量评测、系统状态，并按任务分组；首屏优先展示玻璃壶身开裂/召回评估 P0，移动端改为无横向溢出的底部导航。
- 图标和数据图表分别统一为 Lucide 与 Recharts；机会审核和证据抽屉同步重构，运行 UUID 收入可展开追溯信息。设计 QA 已通过，报告见根目录 `design-qa.md`。
- 当前自动验证：后端 49 条、前端单测 6 条、Playwright 桌面/手机 6 条、真实 Compose 浏览器 1 条全部通过；前端生产构建通过。
